...
public abstract class MediaStream implements Stream {
	...
	public MediaStream() {
		Log.e(TAG, "Suggested mode is " + sSuggestedMode);
	...
	public synchronized void start() throws IllegalStateException, IOException {
		...
		Log.i(TAG, "Value of mMode is:" +mMode);
		if (mMode == MODE_MEDIACODEC_API || mMode == MODE_MEDIACODEC_API_2) {
			encodeWithMediaCodec(CameraActivity.mCamera);
		} else {
	...
	// manually added to pass in camera object
	protected abstract void encodeWithMediaCodec(Camera camera) throws IOException;
